...
def ReadBooks(request):
    ...
    book_list = Book.objects.filter(readers = request.user)
    print(book_list)
    return render(request, 'books/user_profile.html', {'book_list': book_list})
'''
record = []
books = []
ID_MAX = 512
...
def personal_profile(ID):
	...
	print("input the book's title if u want to go to the book's profile, or press E to exit")
	choice = input()
	while 1:
